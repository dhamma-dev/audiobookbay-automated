...
- **Hardcover wanted list** — `/wanted` dashboard syncs "Want to Read" (GraphQL), background-searches ABB (worker thread, broad query ladder). Found rows are **AI-rated once, then settled** (`WANTED_LLM=false` → deterministic). Optional strict auto-download (`WANTED_AUTO_DOWNLOAD`, M4B-only). Dashboard shelves: active / owned (done) / skipped (out of rotation until re-allowed).
